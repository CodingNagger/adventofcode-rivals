...
export default class AdventOfCodeService {
    ...
    async getLeaderboardAsync(config: LeaderboardConfig): Promise<AocPrivateLeaderboard> {
        ...
        var result = await this.getFromUrlAndHeadersAsync<AocPrivateLeaderboard>(
            `https://adventofcode.com/${this.year}/leaderboard/private/view/${config.id}.json`,
    ...
    getFromUrlAndHeadersAsync<T>(uri: string, headers: { [key: string]: string }): Promise<T> {
